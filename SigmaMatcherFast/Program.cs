...
namespace SigmaMatcherFast
{
    class Program
    {
        static int Main(string[] args)
        {
            ...
            using (var engine = new SigmaEngine(rulesDir))
            {
                ...
                DateTime currentA = DateTime.Now;
                ...
                foreach (var file in files)
                {
                    ...
                    {
                        Console.WriteLine($"\n{hit.FilePath} => {hit.LineNo} => {hit.RuleIndex} => {hit.RulePath} => \n{hit.Line}\n");
                    });
                }
                DateTime currentB = DateTime.Now;

                Console.WriteLine($"\n{currentB-currentA} vaqt ketdi.");
            }
            Console.ReadKey();
